 ...
 * @param {HTMLTextAreaElement} textarea - The hidden textarea to sync with.
...
function initializeQuillEditor(
  ...
  // Update textarea on every change to ensure FormData is always current
...
function updateTextareaOnSubmit(form, textarea, quill) {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (textarea.required && !markdownContent) {
      ...
      // Validation message will show on the next attempt if they try to submit empty
      // but we should focus and prevent this one.
      quill.focus();
      event.preventDefault();
      // Reset validity as soon as they type
...
function loadQuillStylesheet() {
  if (document.querySelector('link[href*="quill.snow.css"]')) return;
...
// Observe for dynamic content (e.g. modals)
    ...
    if (mutation.addedNodes.length > 0) {
      for (const node of mutation.addedNodes) {
        if (node.nodeType === Node.ELEMENT_NODE) {
          if (
            (node.tagName === "TEXTAREA" && node.classList.contains("rich-text-editor")) ||
            (node.querySelector && node.querySelector("textarea.rich-text-editor"))
          ) {
            shouldInit = true;
            break;
          }
